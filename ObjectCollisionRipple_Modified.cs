...
namespace Eldvmo.Ripples
{
    public class ObjectCollisionRipple : MonoBehaviour
    {
        ...
        [SerializeField] float rippleRadius = 0.5f; // Giảm 50% bán kính sóng (từ 1.0f xuống 0.5f)
        ...
        void FixedUpdate()
        {
            ...
            if (Physics.Raycast(ray, out hit, 2f, waterLayerMask))
            {
                ...
                // Sử dụng rippleRadius thay vì giá trị cố định 0
                ripplePoints[rippleIndex] = new Vector4(uv.x, uv.y, Time.time, rippleRadius);
